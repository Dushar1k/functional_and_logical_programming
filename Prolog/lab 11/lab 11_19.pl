...
min_down(0,9):- !.                                            min_down(X,N):- X1 is X div 10, Cur is X mod 10, min_down(X1,N
1), ((N1 < Cur, N is N1); N is Cur), !.

nodless3_up(0,N,N):- !.
nodless3_up(X,N,Num):- X1 is X div 10, Cur is X mod 10, ((Cur
< 3, Num1 is Num+1); Num1 is Num), nodless3_up(X1,N,Num1).
num_of_dig_less_3_up(X,N):- nodless3_up(X,N,0), !.

nodless3_down(0,N,0):- !.
nodless3_down(X,N,Num):- X1 is X div 10, Cur is X mod 10, nodl
ess3_down(X1,N,Num1), ((Cur < 3, Num is Num1+1); Num is Num1),
 !.
num_of_dig_less_3_down(X,N):- nodless3_down(X,N,N).

fib_up(N,X,N,X,F2):- !.
fib_up(N,X,I,F1,F2):- F3 is F1+F2, I1 is I+1, fib_up(N,X,I1,F2
,F3).
